...
@WebServlet("/shop/goods_list_shoes")
public class Goods_list_shoes_Controller extends HttpServlet{
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(order==null) {
			...
			new_pro_list =productdao.new_pro_list(5,8);
			req.setAttribute("new_pro_list", new_pro_list);
		}else if(order.equals("2")) {
			Productdao productdao = Productdao.getinstance();
			low_pro_list =productdao.low_pro_list(5,8);
			req.setAttribute("low_pro_list", low_pro_list);
		}else if(order.equals("3")) {
			Productdao productdao = Productdao.getinstance();
			high_pro_list =productdao.high_pro_list(5,8);
			req.setAttribute("high_pro_list", high_pro_list);
		}
		
		req.setAttribute("top", "/shop/header.jsp");
		req.setAttribute("content", "/shop/goods_list_shoes.jsp");
