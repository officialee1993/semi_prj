...
<!-- content -->
<c:choose>
	<c:when test="${param.p_id eq 1}">
	<div class="banner_goods">
		<img src="images/banner_man.png">
	</div>
	</c:when>
	<c:when test="${param.p_id eq 2}">
	<div class="banner_goods">
		<img src="images/banner_woman.png">
	</div>
	</c:when>
</c:choose>
					...
					<div class="goods_list_imgbox_wrap">
						<c:choose>
							<c:when test="${param.order eq 1 }">
								<c:forEach var="vo" items="${new_list_all}">
									<div class="goods_list_imgbox">
										<div><a href="${cp }/shop/goods_detail?p_num=${vo.p_num}"><img src="${cp }/shop/productimgs/${vo.save_img_name }"></a></div>
							...
							<c:when test="${param.order eq 2 }">
								<c:forEach var="vo" items="${low_list_all}">
									<div class="goods_list_imgbox">
										<div><a href="${cp }/shop/goods_detail?p_num=${vo.p_num}"><img src="${cp }/shop/productimgs/${vo.save_img_name }"></a></div>
										<a href="${cp }/shop/goods_detail" class="textbox">
										<span class="title">${vo.p_name }</span>
							...
							<c:when test="${param.order eq 3 }">
								<c:forEach var="vo" items="${high_list_all}">
									<div class="goods_list_imgbox">
										<div><a href="${cp }/shop/goods_detail?p_num=${vo.p_num}"><img src="${cp }/shop/productimgs/${vo.save_img_name }"></a></div>
										<a href="${cp }/shop/goods_detail" class="textbox">
										<span class="title">${vo.p_name }</span>
