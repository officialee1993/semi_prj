				...
				<ul class="goods_list_box_category">
					...
					<li><select class="form-select form-select-sm"
						...
						onchange="if(this.value) location.href=(this.value);">
							...
							<option value="${cp }/shop/goods_list_top?cg_id=3&cg_id1=6&order=1" <c:if test="${optionIndex=='1' }">selected='selected'</c:if>>신상품순</option>
							<option value="${cp }/shop/goods_list_top?cg_id=3&cg_id1=6&order=2" <c:if test="${optionIndex=='2' }">selected='selected'</c:if>>낮은가격순</option>
							<option value="${cp }/shop/goods_list_top?cg_id=3&cg_id1=6&order=3" <c:if test="${optionIndex=='3' }">selected='selected'</c:if>>높은가격순</option>
					</select></li>
				</ul>
